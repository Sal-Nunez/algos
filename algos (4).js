...
class SinglyLinkedList {
    ...
    moveMinToFront() {
        ...
        let runner = this.head
        while (runner.next != null) {
            if (minNode.data > runner.next.data) {
                minNode = runner.next;
                nodeBefore = runner
            }
            runner = runner.next;
        }
        nodeBefore.next = minNode.next;
...
const singleNodeList = new SinglyLinkedList().seedFromArr([1, 2]);
const unorderedList = new SinglyLinkedList().seedFromArr([
    -5, -3, 4, -10, 6, 1, -7, 2,
]);
// (singleNodeList.concat(unorderedList).toArr());
